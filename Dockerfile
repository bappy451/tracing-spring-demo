...
ENTRYPOINT ["java", "-Dserver.port=8080", "-javaagent:opentelemetry-javaagent.jar", "-Dotel.traces.exporter=jaeger", "-Dotel.exporter.jaeger.endpoint=http://tempo.tracing.svc:14250" , "-Dotel.resource.attributes=service.name=new-days", "-Dotel.exporter.jaeger.timeout=30000", "-Dotel.javaagent.debug=false", "-Dotel.resource.attributes=service.name=new-test", "-jar", "app.jar"]
